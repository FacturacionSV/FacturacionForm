...
namespace GeneradorDocumentos
{
    public class GeneradorPdfDte
    {
        ...
        /// <summary>
        /// Genera un PDF de un DTE en memoria
        /// </summary>
        /// <param name="jsonDte">El JSON del DTE</param>
        ...
        public byte[] GenerarPdfEnMemoria(string jsonDte,string sello)
        {
            ...
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                documento.AddTitle("Documento Tributario Electrónico");
        ...
        /// </summary>
        /// <param name="pdfBytes">Bytes del PDF a mostrar</param>
        ...
        private void AgregarEncabezado2(Document documento, dynamic dte)
        {
            PdfPTable tablaEncabezado = new PdfPTable(3);
            tablaEncabezado.WidthPercentage = 100;
            tablaEncabezado.SetWidths(new float[] { 30f, 40f, 30f });

            // Columna 1: Empresa (fuente 10px)
            PdfPCell celdaEmpresa = new PdfPCell();
            celdaEmpresa.Border = Rectangle.NO_BORDER;
            celdaEmpresa.PaddingBottom = 5f;

            Paragraph nombreEmpresa = new Paragraph(dte.emisor.nombre.ToString(), new Font(Font.FontFamily.HELVETICA, 10, Font.BOLD));
            celdaEmpresa.AddElement(nombreEmpresa);

            if (!string.IsNullOrEmpty(dte.emisor.nombreComercial.ToString()))
            {
                Paragraph nombreComercial = new Paragraph(dte.emisor.nombreComercial.ToString(), new Font(Font.FontFamily.HELVETICA, 8));
                celdaEmpresa.AddElement(nombreComercial);
            }

            celdaEmpresa.AddElement(new Paragraph(ObtenerDireccionCompleta(dte.emisor.direccion), new Font(Font.FontFamily.HELVETICA, 8)));
            celdaEmpresa.AddElement(new Paragraph("Tel: " + dte.emisor.telefono, new Font(Font.FontFamily.HELVETICA, 8)));
            tablaEncabezado.AddCell(celdaEmpresa);

            // Columna 2: Título DTE (fuente 12px)
            PdfPCell celdaTitulo = new PdfPCell();
            celdaTitulo.Border = Rectangle.NO_BORDER;
            celdaTitulo.VerticalAlignment = Element.ALIGN_MIDDLE;
            celdaTitulo.AddElement(new Paragraph(ObtenerTipoDte(dte.identificacion.tipoDte.ToString()), new Font(Font.FontFamily.HELVETICA, 12, Font.BOLD)));
            tablaEncabezado.AddCell(celdaTitulo);

            // Columna 3: Info DTE (fuente 8px)
            PdfPCell celdaInfoDte = new PdfPCell();
            celdaInfoDte.Border = Rectangle.BOX;
            celdaInfoDte.BorderColor = BaseColor.BLACK;
            celdaInfoDte.Padding = 5f;

            celdaInfoDte.AddElement(new Paragraph("N° Control: " + dte.identificacion.numeroControl, new Font(Font.FontFamily.HELVETICA, 8, Font.BOLD)));
            celdaInfoDte.AddElement(new Paragraph("Código: " + dte.identificacion.codigoGeneracion, new Font(Font.FontFamily.HELVETICA, 8)));
            celdaInfoDte.AddElement(new Paragraph("Sello: " + sello, new Font(Font.FontFamily.HELVETICA, 8)));
            celdaInfoDte.AddElement(new Paragraph("Fecha: " + FormatearFechaHora(dte.identificacion.fecEmi.ToString(), dte.identificacion.horEmi.ToString()), new Font(Font.FontFamily.HELVETICA, 8)));
            tablaEncabezado.AddCell(celdaInfoDte);
        ...
        private void AgregarEncabezado(Document documento, dynamic dte)
        {
            ...
            //if (!string.IsNullOrEmpty(dte.emisor.nombreComercial.ToString()))
            //    empresa.Add(new Phrase("\n" + dte.emisor.nombreComercial.ToString(), new Font(Font.FontFamily.HELVETICA, 8)));
            ...
            // Título DTE (fuente 12px)
            Paragraph titulo = new Paragraph(ObtenerTipoDte(dte.identificacion.tipoDte.ToString()), new Font(Font.FontFamily.HELVETICA, 12, Font.BOLD));
        ...
        private void AgregarDatosEmisorReceptor(Document documento, dynamic dte)
        {
            ...
            string tipoDoc = ObtenerTipoDocumento(dte.receptor.tipoDocumento.ToString());
            Paragraph documentoReceptor = new Paragraph($"{tipoDoc}: {dte.receptor.numDocumento ?? "No proporcionado"}", _fuenteNormal);
            documentoReceptor.Alignment = Element.ALIGN_LEFT;
            celdaReceptor.AddElement(documentoReceptor);
        ...
        private void AgregarNotasFinales(Document documento, dynamic dte)
        {
            ...
            // Notas legales
            Paragraph notaLegal = new Paragraph("Este documento es una representación gráfica de un Documento Tributario Electrónico (DTE) autorizado por la Dirección General de Impuestos Internos.", _fuentePequena);
        ...
        {
            // Construir la URL oficial de consulta (como la que proporcionaste)
            ...
            // Crear el código QR con la URL oficial (sin texto adicional)
            ...
            //// Texto de verificación (opcional)
            //Paragraph textoVerificacion = new Paragraph(
            //    "Verifique este documento en: " + urlConsulta,
            //    _fuentePequena
            //);
            //textoVerificacion.Alignment = Element.ALIGN_CENTER;
            //documento.Add(textoVerificacion);
        }
        ...
        private string ObtenerTipoDocumento(string codigo)
        {
            switch (codigo)
            {
                case "36": return "NIT";
        ...
        {
            // Este método debería implementarse con un catálogo completo de municipios
            // Por simplicidad aquí solo se muestran algunos ejemplos
